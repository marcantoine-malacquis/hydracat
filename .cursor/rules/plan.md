...
1. yes, "Calendar Legend" (concise and clear)
2. Yes - create a simple expandable structure that makes it easy to add
  sections later.
3. Use Icons.help_outline (outlined circle with "?") positioned in the
  AppBar's actions (top-right)
4. Show the actual colored dot (8x8 circle) next to each status
  description, matching exactly what appears in the calendar
5. Yes (same as day detail popup, dismissible: true). Also include a standard "x" in the top right corner just in case.
