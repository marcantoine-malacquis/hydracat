...
ProviderContainer createTestContainer({
  ...
  when(() => indexStore.getForToday(any<String>(), any<String>()))
      .thenAnswer((_) async => const <ScheduledNotificationEntry>[]);
  when(() => indexStore.putEntry(any<String>(), any<String>(), any()))
      .thenAnswer((_) async {});
  when(() => indexStore.removeEntryBy(
        any<String>(),
        any<String>(),
        any<String>(),
        any<String>(),
  ...
  when(() => indexStore.getCountForPet(
        any<String>(),
        any<String>(),
        any(),
...
void main() {
  ...
  group('ReminderPluginInterface Integration Tests', () {
    ...
    group('Deterministic Notification ID Generation', () {
      test('generateNotificationId produces consistent IDs for same inputs',
          () {
      ...
      test('generateNotificationId produces different IDs for different inputs',
          () {
    ...
    group('Plugin Mock Integration', () {
      test('plugin.showZoned is callable with correct parameters', () async {
        final now = DateTime(2025, 1, 15, 8, 0);
      ...
      test('plugin.pendingNotificationRequests returns empty list by default',
          () async {
        // Execute
      ...
      test('plugin.showGroupSummary is callable with correct parameters',
          () async {
        // Execute
        ...
        // Verify
        verify(() => mockPlugin.showGroupSummary(
              petId: 'pet123',
              petName: 'Fluffy',
              medicationCount: 2,
              fluidCount: 1,
      ...
      test('plugin.canScheduleExactNotifications returns true by default',
          () async {
        // Setup
        when(() => mockPlugin.canScheduleExactNotifications())
    ...
    group('Index Store Mock Integration', () {
      test('indexStore.putEntry is callable with correct parameters',
          () async {
        // Create test entry
        ...
        // Setup
        when(() => mockIndexStore.putEntry(any(), any(), any()))
            .thenAnswer((_) async {});
        ...
        // Verify called
        verify(() => mockIndexStore.putEntry('user1', 'pet1', entry))
            .called(1);
      });

      test('indexStore.removeEntryBy is callable for cancellation', () async {
        // Setup
        when(() => mockIndexStore.removeEntryBy(
              any<String>(),
              any<String>(),
              any<String>(),
              any<String>(),
        ...
        // Verify
        verify(() => mockIndexStore.removeEntryBy(
              'user1',
              'pet1',
              'sched1',
              '08:00',
      ...
      test('indexStore.getForToday returns empty list by default', () async {
        // Setup
        when(() => mockIndexStore.getForToday(any(), any()))
            .thenAnswer((_) async => const []);
      ...
      test('indexStore.getCountForPet returns zero by default', () async {
        ...
        verify(() => mockIndexStore.getCountForPet(any(), any(), any()))
            .called(1);
      });
    });
    ...
    group('Integration: Scheduling Flow with Mocks', () {
      test('scheduling flow updates index after plugin call succeeds',
          () async {
        ...
        verify(() => mockIndexStore.putEntry(
              any<String>(),
              any<String>(),
              any(),
            )).called(1);
      ...
      test('cancellation flow updates index after plugin call succeeds',
          () async {
        final container = createTestContainer(
          mockPlugin: mockPlugin,
          mockIndexStore: mockIndexStore,
