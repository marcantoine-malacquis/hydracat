...
void main() {
  group('ScheduleDateHelpers', () {
    test('hasReminderOnDate returns false when no reminders for non-scheduled day (everyOtherDay)', () {
      final createdAt = DateTime(2025, 10, 10, 10); // Thursday
      final testDate = DateTime(2025, 10, 13, 12); // Sunday (3 days later, not scheduled)
      
      final schedule = Schedule(
        id: 's1',
        treatmentType: TreatmentType.medication,
        frequency: TreatmentFrequency.everyOtherDay,
        reminderTimes: [createdAt.add(const Duration(hours: -1))], // 9:00 AM on creation day
        isActive: true,
        createdAt: createdAt,
        updatedAt: createdAt,
    ...
    test('onceDaily returns reminder time for any date with same time-of-day', () {
    ...
    test('twiceDaily returns both times for requested date', () {
      ...
      expect(times[0].day, 17); // Oct 17, not Oct 10
    ...
    test('everyOtherDay returns times only on scheduled days', () {
      ...
      final times = schedule.todaysReminderTimes(DateTime(2025, 10, 12)).toList();
      expect(times[0].day, 12); // Oct 12, not Oct 10
      expect(times[0].hour, 9);
    ...
    test('every3Days returns times only on scheduled days', () {
      ...
      final times = schedule.todaysReminderTimes(DateTime(2025, 10, 13)).toList();
